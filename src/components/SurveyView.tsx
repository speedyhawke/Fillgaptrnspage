...
import { CheckCircle2, ShieldCheck, MapPin, Send, Sparkles, FileText } from 'lucide-react';
...
export const SurveyView: React.FC<SurveyViewProps> = ({ onNavigateToResources, onOpenDetailedSurvey }) => {
  const gapOptions = [
    'Food',
    'Housing',
    'Employment',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await AdminStore.addCommunitySurvey({
        submittedAt: new Date().toISOString(),
        q1: 'I currently live in Newfoundland and Labrador',
        q2: "St. John's",
        q6: struggledToFind === 'Yes' ? 'Yes' : 'No',
        q7: selectedGaps,
        q8: struggledToFind === 'Yes' ? 'Very difficult' : struggledToFind === 'No' ? 'Easy' : 'Moderate',
        q10: sentFromOneToAnother === 'Yes' ? 'Yes, multiple times' : 'No',
        q12: selectedGaps,
        q13: selectedGaps.slice(0, 3),
        q14: struggledToFind === 'Yes' ? 'Yes' : 'No',
        q15: whatMadeDifficult,
        q22: whatIsMissing,
        q32: oneThingWishExisted,
        q42: whatIsMissing || oneThingWishExisted,
        q43: anythingElse,
      });
    } catch (err) {
      console.warn('Error recording inline community survey:', err);
    } finally {
    ...
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12 space-y-10">
      ...
      {submitted ? (
        ...
        <div className="space-y-6">
          {/* Detailed 11-Section Community Research Survey Callout */}
          <div className="space-y-4">
            <div className="p-6 rounded-3xl bg-gradient-to-r from-[#0e1422] to-[#151c2e] border-2 border-amber-400/50 text-white shadow-xl flex flex-col lg:flex-row items-start lg:items-center justify-between gap-5">
              <div className="space-y-1.5">
                <div className="flex items-center gap-2">
                  <Sparkles className="w-4 h-4 text-[#E5A93C]" />
                  <span className="text-[10px] font-black uppercase tracking-widest text-[#F3BA4F]">
                    Official Community Research
                  </span>
                </div>
                <h2 className="text-lg sm:text-xl font-black text-white uppercase tracking-tight">
                  Official Community Survey (Google Forms & Interactive)
                </h2>
                <p className="text-xs sm:text-sm text-stone-300 max-w-xl leading-relaxed">
                  Have a few minutes? You can complete our official Community Survey directly on Google Forms or launch the interactive 11-section questionnaire right here.
                </p>
              </div>

              <div className="flex flex-wrap items-center gap-3 shrink-0">
                <a
                  href="https://docs.google.com/forms/d/e/1FAIpQLSdO7wZqZ15J4n7h-6aum_LHRuas2yYvKtPBGqSQSh6cxKxQJbIEE/viewform"
                  target="_blank"
                  rel="noopener noreferrer"
                  onClick={(e) => {
                    // Fallback to the provided direct edit/view form link
                    e.currentTarget.href = "https://docs.google.com/forms/d/1nu7-7fW5aum_LHRuas2yYvKtPBGqSQSh6cxKxQJbIEE/viewform";
                  }}
                  className="px-5 py-3 rounded-2xl bg-gradient-to-r from-amber-400 via-amber-500 to-yellow-500 hover:brightness-105 text-slate-950 font-black text-xs uppercase tracking-wider shadow-md flex items-center gap-2 border border-amber-300"
                >
                  <FileText className="w-4 h-4" />
                  <span>Open Official Google Form</span>
                </a>

                {onOpenDetailedSurvey && (
                  <button
                    type="button"
                    onClick={onOpenDetailedSurvey}
                    className="px-5 py-3 rounded-2xl bg-slate-800 hover:bg-slate-700 text-white font-bold text-xs uppercase tracking-wider border border-slate-700 shadow-md flex items-center gap-2 cursor-pointer"
                  >
                    <span>Launch Interactive Survey</span>
                  </button>
                )}
              </div>
            </div>
