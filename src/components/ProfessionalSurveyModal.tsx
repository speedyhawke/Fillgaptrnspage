...
export const ProfessionalSurveyModal: React.FC<ProfessionalSurveyModalProps> = ({
  ...
  const [effectiveCount, setEffectiveCount] = useState(() => AdminStore.getEffectiveProfessionalCount());
  const [isLoadingIframe, setIsLoadingIframe] = useState(true);

  useEffect(() => {
    const handleUpdate = () => {
      setGoogleConfig(AdminStore.getGoogleConfig());
      setEffectiveCount(AdminStore.getEffectiveProfessionalCount());
    };
  ...
  return (
    <div
      id="professional-survey-modal-overlay"
    ...
    >
      <div
        id="professional-survey-modal-content"
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 sm:px-8 py-4 sm:py-5 border-b border-slate-800 bg-slate-950/70 backdrop-blur-md shrink-0">
          <div className="flex items-center gap-3.5">
            <div className="w-10 h-10 rounded-2xl bg-sky-400/10 border border-sky-400/30 flex items-center justify-center text-sky-400 shadow-xs">
              <Briefcase className="w-5 h-5" />
            </div>
            <div>
              <div className="flex items-center gap-2">
                <h2 className="text-lg sm:text-xl font-black text-white tracking-tight">
                  Frontline & Professional Survey
                </h2>
                <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-sky-400/10 text-sky-300 border border-sky-400/20">
                  <Sparkles className="w-3 h-3 text-sky-400" />
                  Service Providers
                </span>
              </div>
              <p className="text-xs text-slate-400">
                Frontline perspective on systemic bottlenecks and inter-agency gaps in NL
              </p>
            </div>
          </div>
        ...
        <div className="px-5 sm:px-8 py-2.5 bg-slate-950/40 border-b border-slate-800/80 flex flex-wrap items-center justify-between gap-3 text-xs text-slate-300">
          <div className="flex items-center gap-4">
            <span className="flex items-center gap-1.5 font-medium text-sky-300">
              ...
              <span>{effectiveCount} Frontline / Agency Responses Logged</span>
            </span>
            <span className="hidden md:inline-flex items-center gap-1 text-slate-400">
              <Clock className="w-3.5 h-3.5" />
              <span>Takes ~4 to 6 minutes</span>
            </span>
          </div>
          ...
          <div className="flex items-center gap-2">
            <span className="inline-flex items-center gap-1 text-[11px] text-slate-400 bg-slate-800/60 px-2.5 py-1 rounded-lg border border-slate-700/50">
              ...
              <span>Confidential • Agency Privacy Protected</span>
            </span>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-2 sm:p-4 bg-slate-900/50 flex flex-col relative min-h-[420px]">
          {isLoadingIframe && (
            <div className="absolute inset-0 flex flex-col items-center justify-center bg-slate-900/90 z-10 gap-3">
              ...
              <p className="text-sm font-medium text-slate-300">Loading Frontline & Agency Survey...</p>
              <button
                onClick={handleOpenExternal}
          ...
          <div className="w-full h-full min-h-[560px] rounded-2xl overflow-hidden border border-slate-800 bg-white shadow-inner flex-1">
            <iframe
              src={embedUrl}
              title="Fill The Gap Frontline & Professional Survey"
        ...
        <div className="px-5 sm:px-8 py-3.5 border-t border-slate-800 bg-slate-950/70 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs text-slate-400">
          <div className="flex items-center gap-2 text-center sm:text-left">
            ...
            <span>Submissions stream directly into the verified frontline consultation spreadsheet.</span>
          </div>

          <div className="flex items-center gap-2.5 w-full sm:w-auto justify-end">
            <button
              onClick={handleOpenExternal}
              className="flex-1 sm:flex-none inline-flex items-center justify-center gap-1.5 px-4 py-2 rounded-xl bg-sky-500 hover:bg-sky-400 text-slate-950 font-bold transition-all shadow-md cursor-pointer"
            >
